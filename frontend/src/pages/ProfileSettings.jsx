...
function ProfileAvatar({ name = '', avatarUrl = '', size = 96 }) {
  ...
  // Semua fallback color dari palet primary
  const colors = ['#5B4FE8', '#4a3fd1', '#6B5FF0', '#3B35B8', '#818CF8'];
...
export default function ProfileSettings() {
  ...
  const handleAvatarChange = async (e) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) return showToast('File harus berupa gambar (JPG, PNG, WebP).', 'error');
            ...
            <div className="flex flex-col sm:flex-row items-center gap-5">
              <div className="relative group cursor-pointer" onClick={handleAvatarClick}>
                ...
                <div className="absolute inset-0 rounded-full bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  {uploadingAvatar ? IcoSpin : <span className="text-white">{IcoCamera}</span>}
                </div>
              </div>
              <div className="text-center sm:text-left">
                ...
                <button
                  ...
                  className="text-xs font-semibold text-[#5B4FE8] hover:underline cursor-pointer bg-none border-none p-0 disabled:opacity-50"
                >
