      ...
      <main className="flex-1 pt-20">
        <div className="max-w-[820px] mx-auto px-6 py-12">
          ...
          <div className="text-center mb-10 animate-fade-up">
            <h1 className="font-display font-extrabold text-[clamp(28px,4vw,44px)] text-[#0F1226] mb-3 leading-tight">
              Analyze your CV<br/>with AI
            </h1>
            <p className="text-[#5A5F7D] text-[15px] max-w-[500px] mx-auto leading-relaxed">
              Our advanced AI will scan your professional profile to identify key skills and match you with Indonesian career opportunities.
            </p>
          </div>
          ...
          {analyzing ? (
            <div className="card-base p-10 text-center mb-6 animate-fade-in">
              <div
                className="w-12 h-12 border-[3.5px] border-[#E8EAF2] rounded-full mx-auto mb-6 animate-spin"
                style={{ borderTopColor: '#5B4FE8' }}
              />
            ...
            <div
              className={`border-2 border-dashed rounded-3xl p-14 text-center cursor-pointer transition-all duration-200 mb-4 animate-fade-up
                ${dragging
                  ? 'border-[#5B4FE8] bg-[#F0EFFE] scale-[1.01]'
                  : file
                  ? 'border-[#5B4FE8] bg-white cursor-default'
                  : 'border-[#D0D3E8] bg-white hover:border-[#5B4FE8] hover:bg-[#F0EFFE]'
                }`}
            ...
            >
              ...
              {file ? (
                <div className="flex items-center gap-4 text-left max-w-[380px] mx-auto">
                  <div className="w-14 h-14 bg-[#F0EFFE] rounded-xl flex items-center justify-center flex-shrink-0">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-[#F8F9FE] text-[#9EA3BC] hover:text-[#5A5F7D] transition-colors cursor-pointer bg-none border-none"
                  >
                ...
                <>
                  <div className="w-16 h-16 bg-[#F0EFFE] rounded-full flex items-center justify-center mx-auto mb-5">
                  ...
                  <div className="flex items-center justify-center gap-3 flex-wrap">
                    {[['Supported format: PDF'], ['Please upload your CV in English']].map(([e], idx) => (
                      <span key={idx} className="flex items-center gap-1.5 text-xs text-[#9EA3BC] bg-[#F8F9FE] border border-[#E8EAF2] px-3 py-1.5 rounded-full">{e}</span>
                    ))}
                  </div>
                </>
              )}
              ...
              {!file ? (
                ...
                <>
                  <button
                    ...
                    className="px-6 py-2.5 rounded-full text-sm font-semibold text-[#5A5F7D] hover:bg-[#E8EAF2] transition-colors cursor-pointer bg-none border-none font-display"
                  >
                    Change file
                  </button>
          ...
          {error && (
            <p className="text-center text-sm text-red-500 bg-red-50 border border-red-100 px-4 py-3 rounded-xl mb-4 animate-fade-in">{error}</p>
          )}

          {/* Feature cards — seragam #F0EFFE / #5B4FE8 */}
          <div className="grid md:grid-cols-3 gap-4 mt-10">
            {FEATURES.map((f, i) => (
              <div key={i} className="card-base p-6">
                <div className="w-10 h-10 bg-[#F0EFFE] text-[#5B4FE8] rounded-xl flex items-center justify-center mb-4">{f.icon}</div>
